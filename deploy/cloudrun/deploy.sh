...
gcloud builds submit \
  ...
  --substitutions="_REGION=$REGION,_REPOSITORY=$AR_REPOSITORY,_NEXT_PUBLIC_SUPABASE_URL=$SUPABASE_URL,_NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=$SUPABASE_PUBLISHABLE_KEY,_NEXT_PUBLIC_NADID_MAX_FILE_MB=50" \
  .
...
gcloud run deploy nadid-web \
  ...
  --min 0 \
  --max 20 \
  --set-env-vars="NEXT_PUBLIC_SUPABASE_URL=$SUPABASE_URL,NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=$SUPABASE_PUBLISHABLE_KEY,AEE_BACKEND_URL=$AEE_URL,NADID_MAX_FILE_MB=50,NEXT_PUBLIC_NADID_MAX_FILE_MB=50,NADID_MAX_UPLOADS_PER_DAY=20,NADID_MAX_ACTIVE_UPLOADS=3" \
  --set-secrets="SUPABASE_SERVICE_ROLE_KEY=nadid-supabase-service-role:latest,AEE_INTERNAL_TOKEN=nadid-aee-internal-token:latest"
